$(document).ready(function() {
    ...
    steamTab.on('mouseenter',()=>{
        clearTimeout(leaveTime)
        enterTime = setTimeout(()=>{ $('#myTab').css('display','flex')}, 200);
    })
    steamTab.on('mouseleave',()=>{
        clearTimeout(enterTime)
        leaveTime = setTimeout(()=>{ $('#myTab').css('display','none')}, 1)
    })
